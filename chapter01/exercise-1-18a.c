/*
 * K&R 1-18 p31: Write a program to remove trailing blanks and tabs
 * from each line of input, and to delte entirely blank lines.
 *
 ...
 * through the file will correct the problem. The program singnals the
...
int main(void)
{
	...
	while((c = getchar()) != EOF)
	{
		if(c == '\n')
		{
			...
			{
				putchar('\n');
				spaceJustPrinted = 0; /* this instruction isn't really neccessary since
										spaceJustPrinted is only used to determine the
										return value, but we'll keep this boolean
										truthful. */
				nonspace = 0; /* moved inside conditional just to save a needless assignment. */
			}
		} else if(c == ' ' || c == '\t')
		{
